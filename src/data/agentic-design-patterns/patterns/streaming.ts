...
export const pattern: Pattern = {
  ...
  whenToUse: [
    ...
    'A good fit when you need to surface mid-generation telemetry (token counts, finish reason, tool calls under construction) that is impossible to inspect once the response is a single returned blob.',
  ...
  realWorldExamples: [
    ...
    {
      text: 'OpenAI\'s cookbook publishes a runnable notebook that opens a chat completion with stream=True, iterates the SSE response, and prints each text delta as it arrives. It is the reference shape every OpenAI-compatible client implements.',
