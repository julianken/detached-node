...
export const pattern: Pattern = {
  ...
  whenToUse: [
    ...
    'A good fit when state has clear schema: episodes you can timestamp, facts you can normalise, procedures you can name and retrieve by intent.',
    'Worth the cost when retention, eviction, and namespacing matter to the product (per-user memory in a multi-tenant assistant, redaction on request, audit trails on what the agent learned and when).',
  ],
  whenNotToUse: [
  ...
  realWorldExamples: [
    {
      text: 'LangGraph ships a long-term memory store keyed by namespace tuple and thread id, exposing get/put/search primitives that agents call inside graph nodes. It is the canonical worked example of the persistent-store half of this pattern.',
      sourceUrl: 'https://langchain-ai.github.io/langgraph/concepts/memory/',
    },
    {
      text: 'Mastra documents an agent memory layer that combines a recent-messages working window, a semantic-recall index for older turns, and a working-memory document the agent rewrites between sessions. The result is a full three-tier implementation behind a single API surface.',
