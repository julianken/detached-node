...
export const pattern: Pattern = {
  ...
  oneLineSummary: 'Fires N LLM calls in parallel and reduces them with a deterministic aggregator.',
  bodySummary: [
  ...
  whenToUse: [
    ...
    'A good fit on questions with verifiable answers where uncorrelated samples raise accuracy past greedy decoding (arithmetic, multiple choice, code generation), as the Self-Consistency line of work demonstrates.',
    'Useful for safety-critical reads where one model handles the user query while a sibling model independently screens it, so the guardrail decision is not entangled with the response prompt.',
  ],
  whenNotToUse: [
  ...
  realWorldExamples: [
    {
      text: 'Anthropic\'s public cookbook ships a runnable parallelization workflow whose top-level helper takes a single prompt and a list of N inputs, runs them concurrently with asyncio.gather, and returns the per-input outputs ready for an aggregator step. It is the exact fan-out shape this pattern names.',
    ...
    {
      text: 'Google ADK exposes a ParallelAgent primitive whose sub-agents run concurrently and write into shared session state, paired with a downstream merger agent that synthesises their outputs. It is a production wiring of the fan-out and aggregator split.',
