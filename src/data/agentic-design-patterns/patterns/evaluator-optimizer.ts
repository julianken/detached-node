...
export const pattern: Pattern = {
  ...
  whenToUse: [
    ...
    'Best for situations where the critic has access to a signal the generator does not (a unit-test runner, a type checker, a search-grounded fact check, or a stronger model graded against a weaker one).',
    'Reach for it when the user is willing to trade two-to-five times the latency and tokens for a measurable lift in correctness on a high-value request, not for low-stakes chat where the first draft is good enough.',
  ],
  whenNotToUse: [
  ...
  realWorldExamples: [
    ...
    {
      text: 'LangChain documents a Reflection-style agent in their reflection-agents post that pairs a generator with a reflector inside a single LangGraph run, looping until the reflector stops returning critique. The wiring is the within-attempt shape this pattern names.',
