...
Migrations are **reconciled and running in production** via the `deploy.yml` pre-build step (since PR #142). The deploy workflow runs `pnpm payload migrate` before the Docker build on every push to `main`. See the [Run Payload migrations step in `.github/workflows/deploy.yml`](../.github/workflows/deploy.yml).

The N-1 Backwards-Compatibility Contract (below) is the source of truth for migration safety.

## Migration Files

Source of truth is `migrations/index.ts`.
...
Migrations run automatically in CI before the Docker build. No manual step is needed.
