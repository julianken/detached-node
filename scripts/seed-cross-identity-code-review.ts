...
async function seed() {
  ...
  const body = createRichTextMulti([
    ...
    'PR #342 (W2.2 Checkpointing Tier A satellite) demonstrates the workflow end-to-end at production scale. The implementer ran on Sonnet; the bot dispatched with `model: opus` and a fresh context window — cross-tier held per R12. The reviewer read the diff cold via `gh pr diff`, ran the mandatory R8 second pass, confirmed all Tier A invariants against the verification ledger (`pnpm test:unit`, `pnpm typecheck`, slug resolution, URL HTTP-200), and posted APPROVE on the first review cycle. R9 attribution distinguished plan-scope from implementer-scope observations in the verdict.',
